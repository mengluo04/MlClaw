...
export interface TaskEvent {
  ...
  taskId: string;
  type: string;
  data: { text?: string; messageId?: string; status?: string; message?: string };
  createdAt: string;
}
...
export interface EmailConfigInput extends EmailConfig {
  password?: string;
}
export interface ChannelAccountView {
  email?: EmailConfig;
  webhook?: { method: WebhookMethod; headerNames: string[]; bodyTemplate: string };
  displayName: string;
  scheduleDelivery: { available: boolean; statusLabel: string };
  id: string;
  kind: ChannelKind;
  remoteId: string;
  enabled: boolean;
  ...
  state: 'stopped' | 'connecting' | 'connected' | 'error' | 'expired';
  message: string;
}
...
export interface WeixinLoginView {
  id: string;
  state: 'wait' | 'scaned' | 'need_verifycode' | 'confirmed' | 'expired' | 'error';
  qrDataUrl: string;
  expiresAt: string;
  message: string;
}
...
export type WebProviderName = 'tavily' | 'firecrawl' | 'exa' | 'brave' | 'searxng';
...
export interface WebConnectionTest {
  ok: true;
  durationMs: number;
  resultCount: number;
}
...
export interface ConversationContext {
  checkpoint?: {
    retainedUsers: { id: string; content: string }[];
    assistantMessageId: string;
    assistantOffset: number;
    assistantPrefixHash: string;
  };
  autoSummary: boolean;
  version: number;
  summary: string;
...
export type SystemLogSource =
  'system' | 'auth' | 'task' | 'schedule' | 'channel' | 'model' | 'web' | 'executor' | 'storage';
export interface SystemLogRecord {
  ...
  event: string;
  message: string;
  entityType: 'task' | 'schedule' | 'occurrence' | 'channel' | 'tool' | null;
