...
const load = async (older = false) => {
  ...
  if (query.value.trim()) {
    ...
    /** 接口 /tasks/search?${params} 返回的业务数据。 */
    ...
    /** 接口 /tasks/page?${params} 返回的业务数据。 */
    const result = await api<{ items: Row[]; nextCursor: string | null }>(`/tasks/page?${params}`);
...
<template>
  <div class="page-content feature-panel">
    <section>
      ...
      >
      <p v-if="searched">
        关键词搜索仅包含已结束任务，最多 10 条。{{ truncated ? '结果已截断，请细化关键词。' : '' }}
      </p>
      <el-skeleton v-if="busy && !loaded" :rows="5" animated /><el-table
        ...
        ><el-table-column label="操作" width="160"
          ><template #default="{ row }"
            ><el-button text :disabled="busy" @click="inspect(String(row.id))">详情</el-button
      ...
      <div class="mobile-data">
        <el-empty v-if="loaded && !items.length" description="暂无符合条件的记录" />
      ...
      <el-button v-if="nextCursor" :loading="busy" @click="run(() => load(true))"
        >加载更早记录</el-button
      >
      <el-drawer v-model="detailOpen" title="任务详情" size="620px" destroy-on-close
        ><div v-if="detail" class="feature-panel">
          ...
          <p v-if="detail.model_snapshot">实际模型：{{ modelLabel(detail.model_snapshot) }}</p>
          <el-alert
            v-if="detail.error"
            :title="detail.error"
            type="error"
            :closable="false"
          /><el-empty v-if="!detail.tools?.length" description="此任务没有工具调用" /><el-collapse
            ><el-collapse-item
